...
export async function Adminvalidate(req, res, next) {
...
export async function Managervalidate(req, res, next) {
  const sess = req.session.newsession;
  const userid = req.session.userid;
  if (!sess) {
    return next({
...
export async function Usersvalidate(req, res, next) {
  const sess = req.session.newsession;
  const userid = req.session.userid;
...
export async function Shippervalidate(req, res, next) {
  const sess = req.session.newsession;
  const userid = req.session.userid;
  if (!sess) {
    console.log("sess no");
...
export async function active(req, res, next) {
  ...
  if (isLoggedIn(req) === true) {
    // get time stamp NOW
    const now = Date.now();
    const twentyfive = 3600000 * 60 * 10;
